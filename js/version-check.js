...
async function checkForUpdates() {
    try {
        // 获取当前版本
        const currentVersion = await fetchVersion('/VERSION.txt', '取得目前版本失敗', {
            cache: 'no-store'
        });
        ...
        try {
            ...
            console.log('透過代理伺服器取得版本成功');
        } catch (error) {
            console.log('代理請求失敗，嘗試直接請求:', error.message);
            try {
                // 代理失败后尝试直接获取
                latestVersion = await fetchVersion(VERSION_URL.DIRECT, '取得最新版本失敗');
                console.log('直接請求獲取版本成功');
            } catch (directError) {
                console.error('所有版本檢查請求都失敗:', directError);
