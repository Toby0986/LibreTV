    ...
    try {
        if (url.pathname === '/api/search') {
            ...
            if (source === 'custom' && !customApi) {
                throw new Error('使用自訂API時必須提供API位址');
            }
...
async function handleCustomApiSpecialDetail(id, customApi) {
    try {
        ...
        if (!response.ok) {
            throw new Error(`自訂API詳情頁請求失敗: ${response.status}`);
        }
        ...
        console.error(`自訂API詳情取得失敗:`, error);
        throw error;
    }
}
...
async function handleAggregatedSearch(searchQuery) {
    ...
    const searchPromises = availableSources.map(async (source) => {
        try {
            ...
            if (!data || !Array.isArray(data.list)) {
                throw new Error(`${source}來源傳回的資料格式無效`);
            }
...
async function handleMultipleCustomSearch(searchQuery, customApiUrls) {
    ...
    if (apiUrls.length === 0) {
        throw new Error('沒有提供有效的自訂API位址');
    }
    ...
    const searchPromises = apiUrls.map(async (apiUrl, index) => {
        try {
            ...
            // 使用Promise.race添加超时处理
            const timeoutPromise = new Promise((_, reject) => 
                setTimeout(() => reject(new Error(`自訂API ${index+1} 搜尋超時`)), 8000)
            );
            ...
            if (!response.ok) {
                throw new Error(`自定义API ${index+1} 請求失敗: ${response.status}`);
            }
            
            const data = await response.json();
            
            if (!data || !Array.isArray(data.list)) {
                throw new Error(`自訂API ${index+1} 傳回的資料格式無效`);
            }
            ...
            console.warn(`自訂API ${index+1} 搜尋失敗:`, error);
            return []; // 返回空数组表示该源搜索失败
        }
    });
    ...
    try {
        ...
        console.error('自訂API聚合搜尋處理錯誤:', error);
        return JSON.stringify({
            code: 400,
            msg: '自訂API聚合搜尋處理失敗: ' + error.message,
            list: []
        });
    }
}
